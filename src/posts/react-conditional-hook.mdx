...
### useContext는 조건부로 사용이 가능하다 (사실 가능하지 않다고함)
...
위 코드에서는 렌더 사이클상 에러를 내지 않고 있을 뿐 useContext를 조건부를 쓰는 것도 규칙 위반이라고 한다. 최근 나온 useContext의 대체인 use는 조건부 사용이 가능하다고 한다 (https://ko.react.dev/reference/react/use#reading-context-with-use)
